...
CONCERNS = [
    "persistent cough", "intermittent headache", "fatigue", "lower back pain",
    "shortness of breath", "abdominal discomfort", "dizziness", "sore throat",
    "knee pain", "palpitations",
]

CSV_FIELDS = [
    "record_id", "patient_name", "mrn", "dob", "gender", "record_date",
    "record_type", "title", "encounter_id", "text",
]


def build_raw_demo_records() -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    """Return 62 raw synthetic records; two are deliberate duplicates."""
    ...
    for i, (name, mrn, dob, gender) in enumerate(PATIENTS, 1):
        ...
        if i == 13:
            note.pop("id")  # deliberate missing source identifier
        json_records.append(note)

        report_type = "lab result" if i % 2 else "imaging report"
        if report_type == "lab result":
        ...
        followup_mrn = mrn
        if i in (3, 8, 13, 18):
            followup_mrn = ""  # deliberate missing MRN
        if i in (5, 9, 17):
            followup_mrn = str(90000000 + i)  # deliberate conflicting MRN

        normalized_gender = {
            "F": "female", "M": "male", "U": "unknown", "female": "F",
            "male": "M", "2": "F", "1": "M", "nonbinary": "other",
        }.get(gender, gender)
        followup_date = (
...
def write_demo_corpus(data_dir: str | Path = "data") -> tuple[Path, Path]:
